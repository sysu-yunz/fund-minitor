...
func GetMonthProfitRate(fc string) float64 {
	rb := getHistoryRaw(fc, 20)
	...
	lsjzList := rc.Data.LSJZList
	listLen := len(lsjzList)
	if listLen > 0 {
		nowStr := lsjzList[0].DWJZ
		prevStr := lsjzList[listLen-1].DWJZ
		now, err1 := strconv.ParseFloat(nowStr, 64)
		prev, err2 := strconv.ParseFloat(prevStr, 64)
		if err1 != nil  || err2 != nil {
			log.Error("Parse string to float error", err1, err2)
		}
